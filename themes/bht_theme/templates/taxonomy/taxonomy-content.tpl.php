...
<?php if (!empty($nodes)): ?>
  <?php $children = element_children($nodes); ?>
  <section <?php print drupal_attributes($attributes_array); ?>>
    <?php $i = 0; ?>
    <?php foreach($children as $child):  ?>
      <?php $item_attributes['class'] = array(); ?>
      <?php $css_modifier = ''; ?>
      <?php if (isset($nodes[$child]['#view_mode'])) $css_modifier = drupal_clean_css_identifier($nodes[$child]['#view_mode']); ?>
      ...
      <?php $nodes[$child]['#node']->attributes_array = $item_attributes; ?>
      <?php print drupal_render($nodes[$child]); ?>
    <?php endforeach; ?>
  </section>
<?php endif; ?>

...
<?php if (isset($items) && !empty($items)): ?>
  ...
  <?php foreach ($items as $delta => $item): ?>
    ...
    <?php if (isset($item['#markup']) && strlen($item['#markup']) > 0): ?>
      <?php $item['#markup'] = l($item['#markup'], 'tel:' . str_replace(array('(0)', ' '), array('', ''), $item['#markup']), array('attributes' => array('class' => array('contact__link', 'contact__link--phone')), 'html' => FALSE, 'external' => TRUE)); ?>
    <?php endif; ?>
